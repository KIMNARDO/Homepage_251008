...
const HeroSection: React.FC<HeroSectionProps> = ({ content: propContent }) => {
  ...
  const [dataJsonHero, setDataJsonHero] = useState<any>(null);
  ...
  const loadHeroData = () => {
    axios.get('http://localhost:8080/api/public/hero')
      .then((response) => {
        console.log('[HeroSection] Loaded from data.json:', response.data);
        setDataJsonHero(response.data);
      })
      .catch((error) => console.error('[HeroSection] Failed to load:', error));
  };
  ...
  useEffect(() => {
    ...
    const handleHeroUpdate = (event: CustomEvent) => {
      console.log('[HeroSection] Hero updated:', event.detail);
      loadHeroData(); // 새로운 데이터 다시 로드
    };

    window.addEventListener('heroUpdated', handleHeroUpdate as EventListener);

    return () => {
      window.removeEventListener('heroUpdated', handleHeroUpdate as EventListener);
    };
  }, []);
  ...
  const dataJsonContent = dataJsonHero ? {
    announcement: { text: '', href: '' },
    heading: dataJsonHero.title,
    subheading: dataJsonHero.subtitle,
    tagline: '',
    cta: [{ text: dataJsonHero.ctaText, href: dataJsonHero.ctaLink, variant: 'primary' as const }],
    backgroundImage: dataJsonHero.imageUrl
  } : null;

  const content = dataJsonContent || propContent || adminContent || {
  ...
  console.log('[HeroSection] Content:', {
    hasDataJson: !!dataJsonContent,
    hasProp: !!propContent,
    hasAdmin: !!adminContent,
